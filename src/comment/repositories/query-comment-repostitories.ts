...
export const QueryCommentsRepositories={
    async get(queryDto:any,postId:string):Promise<{items:WithId<CommentViewModel>[],totalCount:number}> {
    ...
    async getCommentById(id:string):Promise<WithId<CommentViewModel>|null>{
        ...
        const comments = await commentsCollection.findOne({id:id},{projection:{postId:0}})
        if(!comments){
