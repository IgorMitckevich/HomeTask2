...
export const postsValidationTitle=body('title')
    ...
    .isLength({min:1,max:30}).withMessage('title length must be not more than 30 sybmols')
    .isNumeric().withMessage('title must be a numeric string')
...
export const postsValidationShortDescription=body('shortDescription')
    ...
    .isLength({min:1,max:100}).withMessage('shortDescription length must be not more than 100 sybmols')

export const postsValidationContent=body('content')
    .exists().withMessage('content is required')
    .isString().withMessage('content must be a string')
    .isLength({min:1,max:1000}).withMessage('content length must be not more than 1000 sybmols')
