...
export function mapCommentsPagination(
  posts: { items: WithId<CommentViewModel>[]; totalCount: number },
  pagesNumber: number,
  pageSize: number,
): Pagginator<CommentViewModel> {
  const mapCommentViewModel: CommentViewModel[] = posts.items.map(
    (com: WithId<CommentViewModel>) => {
      return {
        id: com.id,
        content: com.content,
